...
public class Access {
  ...
  /**
   * Tells whether a given access modifier is synthetic. A modifier is synthetic if it is marked with the
   * ACC_SYNTHETIC flag (0x1000), as specified in JLS8, 4.6 <tt>Methods</tt>.
   *
   * @param mod The access modifier to check
   ...
   * (0x0040), as specified in JLS8, 4.6 <tt>Methods</tt>.
   *
   * @param mod The access modifier to check
  ...
  public static void generalize(BClass c) {
    c.access = publicAndGeneric(c.access);
    if (!Access.isInterface(c.access)) {
      c.methods.forEach(m -> m.access = publicAndGeneric(m.access));
      c.fields.forEach(f -> f.access = publicAndGeneric(f.access));
    }
  }

  public static int publicAndGeneric(int access) {
    return Access.removeAccess(Access.makePublic(access), ACC_FINAL, ACC_SYNTHETIC);
  }

  public static int makeProtected(int access) {
    if (Access.isPrivate(access)) {
      access = removeAccess(access, ACC_PRIVATE);
    }
    if (Access.isPublic(access)) {
      access = removeAccess(access, ACC_PUBLIC);
    }
    if (!Access.isProtected(access)) {
  ...
  public static boolean isExtendable(int access) {
    return isPublic(access) && !isFinal(access) && !isAbstract(access) && isInterface(access) && !isNative(access);
  }
