 ...
 */
public class ConfigEquivalenceClassExclusionPassThrough<E extends BMember, T extends IEquivalenceClass<E>> extends ConfigExclusionPassThrough<T> {

  public ConfigEquivalenceClassExclusionPassThrough(Config taskConfig, DataProvider dataProvider) {
  ...
  public Stream<T> passThrough(Stream<T> t) {
    ...
    Set<T> excludedMember = input.stream().filter(ec -> ec.getMembers().stream().anyMatch(bMeth -> matchesAnyToken(getExcludedTokens(), bMeth))).collect(Collectors.toSet());
    Set<T> includedMember = input.stream().filter(ec -> ec.getMembers().stream().anyMatch(bMeth -> matchesAnyToken(getIncludedTokens(), bMeth))).collect(Collectors.toSet());
