...
public class TermUShr extends BiTerm {

  public TermUShr(Term term1, Term term2) {
    super(term1, term2);
  }

  public static TermUShr forNumber(Number n) {
    if (n instanceof Long) {
      long number = n.longValue();
      if (number == 0) {
        long random = R.nextInt(Byte.MAX_VALUE);
        int possibleShift = Bits.shiftCountForZero(random).b;
        possibleShift += R.nextInt(512) * 64;
        return new TermUShr(new NumTerm(random), new NumTerm(possibleShift));
      }
      int possibleShift = Bits.shiftRange((long) n).b;
      int shift = possibleShift > 1 ? 1 + R.nextInt(possibleShift - 1) : 0; // avoid zero
      long l1 = number << shift;
      shift += R.nextInt(512) * 64;
      return new TermUShr(new NumTerm(l1), new NumTerm(shift));
    } else {
      int number = n.intValue();
      if (number == 0) {
        int random = R.nextInt(Byte.MAX_VALUE); // first bit would be illegal
        int possibleShift = Bits.shiftCountForZero(random).b;
        possibleShift += R.nextInt(512) * 32;
        return new TermUShr(new NumTerm(random), new NumTerm(possibleShift));
      }

      int possibleShift = Bits.shiftRange((int) n).b;
      int shift = possibleShift > 1 ? 1 + R.nextInt(possibleShift - 1) : 0; // avoid zero
      int i1 = number << shift;
      shift += R.nextInt(512) * 32;
      return new TermUShr(new NumTerm(i1), new NumTerm(shift));
    }
  }
  ...
  public InsnList getTerm(boolean enhanced) {
    ...
    il.add(termFst);
    il.add(termSnd);
    il.add(new InsnNode(term1.isWide() ? LUSHR : IUSHR));
    return il;
  }
