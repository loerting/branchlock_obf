package net.branchlock.cryptography;

import javax.crypto.*;
import javax.crypto.spec.PBEKeySpec;
import javax.crypto.spec.SecretKeySpec;
import java.nio.charset.StandardCharsets;
import java.security.InvalidKeyException;
import java.security.NoSuchAlgorithmException;
import java.security.spec.InvalidKeySpecException;
import java.security.spec.KeySpec;
import java.util.Base64;
import java.util.Random;

/**
 * Not the most secure cipher, but secure enough and more important: short output.
 */
public class DESedeECB {
  public static final int[] FTABLE = {};
  private final SecretKeySpec secretKey;
  private final int lineEncryptionRounds;

  public DESedeECB(String key) {
    secretKey = initKey(key);
    lineEncryptionRounds = 512;
  }

  private static SecretKeySpec initKey(String key) {
    try {
      SecretKeyFactory factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256");
      KeySpec spec = new PBEKeySpec(key.toCharArray(), generateSalt(key), 65536, 192);
      SecretKey tmp = factory.generateSecret(spec);
      return new SecretKeySpec(tmp.getEncoded(), "DESede");
    } catch (Exception e) {
      throw new IllegalStateException("Key creation failed", e);
    }
  }

  /**
   * Should return the same byte[] for the same key.
   */
  private static byte[] generateSalt(String key) {
    Random sr = new Random(key.hashCode() + ((long) "petebrah".hashCode()) << 32);
    byte[] b = new byte[16];
    sr.nextBytes(b);
    return b;
  }

  public static void main(String[] args) throws Exception {
    String key = "abc";
    DESedeECB des = new DESedeECB(key);
    String hello_world = des.encrypt("Hello World");
    System.out.println(hello_world);
    System.out.println(des.decrypt(hello_world));
  }

  public String encrypt(String strToEncrypt) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException {
    Cipher cipher = Cipher.getInstance("DESede/ECB/PKCS5Padding");
    cipher.init(Cipher.ENCRYPT_MODE, secretKey);
    return Base64.getEncoder().encodeToString(cipher.doFinal(strToEncrypt.getBytes(StandardCharsets.UTF_8)));

  }

  public String decrypt(String strToDecrypt)
    throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException, InvalidKeySpecException {
    Cipher cipher = Cipher.getInstance("DESede/ECB/PKCS5Padding");

    cipher.init(Cipher.DECRYPT_MODE, secretKey);
    return new String(cipher.doFinal(Base64.getDecoder().decode(strToDecrypt)));
  }

  public int encryptLine(int line) {
    byte[] key = secretKey.getEncoded();
    short u2Line = (short) line;
    for (int i = 0; i < lineEncryptionRounds; i++) {
      int firstAddition = u2Line + (short) key[(i ^ lineEncryptionRounds) % key.length];
      firstAddition ^= FTABLE[(256 - key[i % key.length]) % FTABLE.length] * i;
      u2Line = (short) (firstAddition ^ (short) key[i % key.length]);
    }
    // old algorithm allowed minus one.
    return u2Line + ((32768 /* 2 ^ 15 */));
  }

  public int decryptLine(int line) {
    byte[] key = secretKey.getEncoded();
    line -= (32768 /* 2 ^ 15 */);
    short u2Line = (short) line;
    for (int i = lineEncryptionRounds - 1; i >= 0; i--) {
      int xorReversal = u2Line ^ (short) key[i % key.length];
      xorReversal ^= FTABLE[(256 - key[i % key.length]) % FTABLE.length] * i;
      int additionReversal = xorReversal - (short) key[(i ^ lineEncryptionRounds) % key.length];
      u2Line = (short) (additionReversal);
    }
    return u2Line;
  }
}
